<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
      if (isset($_POST["id"]) && !empty(trim($_POST["id"]))) {
        $sql = 'UPDATE orderDetail SET customer = ?, phone = ?, address = ?, city = ?, addressType = ? WHERE id = ?';
        if($stmt = mysqli_prepare($con, $sql)){
          mysqli_stmt_bind_param($stmt, "ssssss", $_POST["customer"], $_POST["phone"], $_POST["address"], $_POST["city"], $_POST["addressType"], $_POST["id"]);
          if(mysqli_stmt_execute($stmt)){
